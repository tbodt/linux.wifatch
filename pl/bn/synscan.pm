...
{                          # dstport, rate=B/s
	...
	my $filter = pack "(S!CCI!)*",
		...
		0x15, 0, 3, SEQ + 1,
		0x40, 0, 0, 0x00000000,
		0x15, 0, 1, ($dstport << 16) | $srcport,
		0x06, 0, 0, 0x0000ffff,
		0x06, 0, 0, 0x00000000,
		;

	setsockopt $sock, Socket::SOL_SOCKET, Socket::SO_ATTACH_FILTER,
		pack "S x![P] P", 9, $filter
	...
	$rw = AE::io $sock, 0, sub {
	  ...
	  # we could assume fixed header size, as "ip options are not an option"
			substr $buf, 0, (15 & ord $buf) * 4,
				"";    # erase ip header

			next
				unless 0x12 ==
				(0x3f & ord substr $buf, 13);    # SYN+ACK
...
sub send
{
	...
	for my $dstip (@_) {
		...
		until (send $sock, (substr $msg, 12),
			0, Socket::pack_sockaddr_in 0, $dstip) {
			if ($! == Errno::ENOBUFS || $! == EAGAIN || $! == EINTR)
			{
				Coro::AnyEvent::sleep 0.01;
			} else {
				bn::log "WARN synscan send: $! - ",
